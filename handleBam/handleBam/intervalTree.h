...
{
#endif
template < class Scalar, typename Value > class Interval
{
public:
    Scalar start;
    Scalar stop;
...
template < class Scalar, typename Value > Value intervalStart(const Interval< Scalar, Value >& i)
{
    return i.start;
}

template < class Scalar, typename Value > Value intervalStop(const Interval< Scalar, Value >& i)
{
    return i.stop;
}
...
template < class Scalar, class Value > class IntervalTree
{
public:
    ...
    IntervalTree(const IntervalTree& other)
        : intervals(other.intervals), left(other.left ? other.left->clone() : nullptr),
          right(other.right ? other.right->clone() : nullptr), center(other.center)
    {
    }
    ...
    template < class UnaryFunction > void visit_near(const Scalar& start, const Scalar& stop, UnaryFunction f) const
    {
    ...
    template < class UnaryFunction > void visit_overlapping(const Scalar& pos, UnaryFunction f) const
    {
    ...
    interval_vector findOverlapping(const Scalar& start, const Scalar& stop) const
    {
        ...
        visit_overlapping(start, stop, [&](const interval& interval) { result.emplace_back(interval); });
        return result;
    }
    ...
    template < class UnaryFunction > void visit_all(UnaryFunction f) const
    {
        if (left)
        {
            left->visit_all(f);
